...
// commonjs
if(typeof module!=="undefined"){module.exports=loadCSS}else{w.loadCSS=loadCSS}})(typeof global!=="undefined"?global:this);$(function(){var btn=$("[data-featured]"),target=$(btn.data("featured")),ani="bounceIn",dance=function(){target.addClass("animated "+ani);setTimeout(function(){target.removeClass("animated "+ani)},750)};btn.on("click hover",dance)});$(window).load(function(){var library="//maxcdn.bootstrapcdn.com/font-awesome/4.4.0/css/font-awesome.min.css";$('<link rel="stylesheet" type="text/css" href="'+library+'" >').appendTo("head")});var Sidebar=function(elem){var author=$('meta[name="author"]').attr("content"),description=$('meta[name="notice"]').attr("content"),title=author+" - "+description;this.open=function(e){e.preventDefault();var proj=this;elem.removeClass("loaded").addClass("opened loading");elem.find(".post-container").load(proj.href+" .post-container .wrapper",function(){elem.removeClass("loading").addClass("loaded");history.pushState({url:proj.href,title:proj.title+" - "+title},proj.title,proj.href);document.title=history.state.title})};this.close=function(){elem.removeClass("opened loaded");history.pushState({url:"/",title:title},title,"/");document.title=history.state.title};/* Main Init */
